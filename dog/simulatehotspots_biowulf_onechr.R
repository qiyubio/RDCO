simulatehotspots_biowulf <- function (
  ...
  tss_weight     = 0,
  h3k4_weight     = 0,
  target_hotspot  = "dog1_noC_merge_longest_peak_noRC"
  #useStrength    = TRUE,
  #useTel_dis     = TRUE,
  #repTimingData  = NULL,
  #IODtime        = 50
...
#if (tss_weight == TRUE){
tss<-read.table(paste('tss_center_canfam3_nochrX.bed.flip.bed_CG_all_distance.txt.flip.bed_',chrs,sep=''),header=FALSE)
...
#if (h3k4_weight==TRUE){
h3k4<-read.table(paste('dog1_h3k4me3_middle.bed_CG_all_distance.txt.flip.bed_',chrs,sep=''),header=FALSE)
...
new$newweight<-new$weight+new$tel_weight2+new$tss_weight2+new$h3k4_weight2
######################################################################################################
...
if(!file.exists(paste(nI,"simulation_CGI_binorm_750",weinew,'read',sam,'dsb',dsb,chrs,".bed",sep="_"))){
 
for (i in 1:nI){
  

  randPositions <- sample((file$cen),dsb,prob=file$newweight)
  for (x in randPositions){
    ...
    for (y in c(randReads,randReads2)){
      if (y+50<0){
        write(paste(chrs,x+y,x+y+50,"reverse","+",sep="\t"),file=paste(nI,"simulation_CGI_binorm_750",weinew,'read',sam,'dsb',dsb,chrs,".bed",sep="_"),append=TRUE)
      }
      if (y>0){
        write(paste(chrs,x+y,x+y+50,"forward","-",sep="\t"),file=paste(nI,"simulation_CGI_binorm_750",weinew,'read',sam,'dsb',dsb,chrs,".bed",sep="_"),append=TRUE)
      }
...
system(paste('sh bed_biowulf_onechr.sh ',nI,'_simulation_CGI_binorm_750_',weinew,'_read_',sam,'_dsb_',dsb,'_',chrs,'_.bed ',' ',chrs,' ',chromSize,' ',target_hotspot,sep=''))

##system(paste('perl ./calcStrengthAndRecenterHotspots.pl --hs dog_peak_',chrs,' --frag ',nI,'_simulation_CGI_binorm_750_',weinew,'_read_',sam,'_dsb_',dsb,'_',chrs,'_.bed.bed',' --noRC ',' --out ',nI,'_simulation_CGI_binorm_750_',weinew,'_read_',sam,'_dsb_',dsb,'_',chrs,'_.bed.bed_peak.bed',sep=''))
#suml<-countLines(paste(nI,'_simulation_CG_',weinew,'_read_',sam,'_dsb_',dsb,'_',chrs,'_.bed.bed',sep=''))
#system(paste('coverageBed -a ',nI,'_simulation_CG_',weinew,'_read_',sam,'_dsb_',dsb,'_',chrs,'_.bed.bed', ' -b dog_peak_',chrs,' > ',nI,'_simulation_CG_',weinew,'_read_',sam,'_dsb_',dsb,'_',chrs,'_.bed.bed_peak.bdg',sep=''))
  
##ol<-readf(paste(nI,'_simulation_CGI_binorm_750_',weinew,'_read_',sam,'_dsb_',dsb,'_',chrs,'_.bed.bed_peak.tab',sep=''))
...
TP<-countLines(paste(nI,'_simulation_CGI_binorm_750_',weinew,'_read_',sam,'_dsb_',dsb,'_',chrs,'_.bed_overlap.bed',sep=''))
FP<-countLines(paste(nI,'_simulation_CGI_binorm_750_',weinew,'_read_',sam,'_dsb_',dsb,'_',chrs,'_.bed_nonoverlap.bed',sep=''))
...
#dog1<-readc("120229_0179_1_NoIndex_FS_021712_dog1_dmc1.canfam3_flip.ssDNA_type1.bed.60_60.bed_normalized_100kb.txt_coverage_merge.bed")
...
#options(scipen=10000)
################################read 100kb coverage############################################
##sim<- readc(paste(nI,'_simulation_CGI_binorm_750_',weinew,'_read_',sam,'_dsb_',dsb,'_',chrs,'_.bed_100kb.bedgraph',sep=''))
...
##  xlab(" ")
##ggsave(plot=d,filename=paste(nI,'_simulation_CGI_binorm_750_',weinew,'_read_',sam,'_dsb_',dsb,'_',chrs,'.bed_100kb','_coverabe_compare.tiff',sep=""),height=6,width=16)
